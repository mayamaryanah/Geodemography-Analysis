...
##menggunakan list
res_fpa <- lapply(2:10, function(j) {
  lapply(1:5, function(k){
    ...
    fpa_param <- c(vi.dist='normal',npar=5, same =15, p=0.7, gamma =1.2, 
                   lambda=1.5,ei.distr='logchaotic', chaos=3, map =0.6)
    res_fpa <- fgwc(data_risk,pop, distance2,'fpa',param_fgwc,fpa_param)
  })
})
...
clus <- c()
for (o in 1:9) clus <- c(clus, res_fpa[[o]][[1]]$cluster)
clus

objf <- c()
for (i in 1:9) objf <- c(objf,res_fpa[[i]][[1]]$f_obj)
objf

val <- c()
for (j in 1:9) val <- c(val, res_fpa[[j]][[1]]$validation)
val

iter <- c()
for (k in 1:9) iter <- c(iter, res_fpa[[k]][[1]]$iteration)
iter

time <- c()
for (l in 1:9) time <- c(time, res_fpa[[l]][[1]]$time)
time


member <- c()
for (m in 1:9) member <- c(member, res_fpa[[m]][[1]]$membership)
member

final <- c()
for (l in 1:9) final <- c(final, res_fpa[[l]][[1]]$finaldata)
final

#####
library(rJava)
library("xlsx")
mean_ifa <- aggregate(.~cluster,res_fpa[[2]][[1]]$finaldata,mean)
mean_pso
write.xlsx(mean_fpa, file = "cluster_mean_ifa.xlsx",
           sheetName = "hasilcluster", append = FALSE)


finaldata <- cbind.data.frame(data_merge,
                              cluster = res_fpa[[2]][[1]]$cluster)
finaldata

##Write file excel from final data

write.xlsx(finaldata, file = "finaldata_fpa.xlsx",
           sheetName = "hasilcluster", append = FALSE)
